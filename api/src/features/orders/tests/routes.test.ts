...
const MOCK_ORDERS: StoredOrder[] = [
  {
    ...
    total: 100,
    orderDate: new Date(),
    products: [{ id: 1, name: 'phone', price: 123, quantity: 1 }],
  },
];
...
describe('Orders route', () => {
  ...
  it('POST / - idempotency', async () => {
    jest.spyOn(db, 'getOrders').mockResolvedValueOnce(MOCK_ORDERS);
    jest.spyOn(db, 'getProductAndPrice').mockResolvedValueOnce(['phone', 123]);
  ...
  it('GET & POST / - cache invalidation', async () => {
    ...
    const res = await request(app).get('/api/orders').set('Authorization', `Bearer ${token}`);
    expect(res.status).toEqual(200);
    expect(res.body[0].products[0].price).toEqual(123);
    ...
    expect(resGet2.body[0].products[0].price).toEqual(123);
